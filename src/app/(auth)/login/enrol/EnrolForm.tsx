...
export function EnrolForm({ qr, secret }: { qr: string; secret: string }) {
  ...
  if (state?.codes) {
    return (
      ...
      >
        <ul className="grid grid-cols-2 gap-1.5 rounded-md border border-rule bg-well p-3 font-mono text-xs">
        ...
        <a
          href="/"
          className="mt-4 flex h-8 w-full items-center justify-center rounded-md bg-accent text-[13px] font-semibold text-accent-ink"
        >
          I have saved them
        </a>
      </AuthShell>
  ...
  return (
    <form action={action}>
      ...
      >
        ...
        <img src={qr} alt="Two-factor setup code" className="mb-3 rounded-md border border-rule" />
        <p className="mb-3 font-mono text-[11px] break-all text-ink-3">{secret}</p>
